...
class NetworkManager: NSObject {
    ...
    func dataTask(
        with request: URLRequest, completion: @escaping (Data?, URLResponse?, Error?) -> Void
    ) -> URLSessionDataTask {
...
extension NetworkManager: URLSessionDelegate {
    ...
    func urlSession(
        ...
        #if DEBUG
            // Debug-only bypass for development with self-signed certificates
            // Tailscale hostname, IP fallback, and local development
            if host == "localhost" || host == "127.0.0.1"
                || host == "desktop-p1p7892.taildbc5d3.ts.net"
                || host == "100.96.92.39"  // Fallback IP
...
extension NetworkManager {
    ...
    /// Perform GET request
    func get(
        url: URL, headers: [String: String]? = nil,
    ...
    func post(
        url: URL, body: Data?, headers: [String: String]? = nil,
        completion: @escaping (Result<Data, Error>) -> Void
    ) {
        var request = URLRequest(url: url)
