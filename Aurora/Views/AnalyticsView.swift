            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 20) {
                    ...
                    VStack(spacing: 16) {
                        MetricRow(
                            label: "Peak Balance",
                            value: String(format: "$%.2f", botManager.peakBalance))
                        MetricRow(
                            label: "Current Drawdown",
                            value: String(format: "%.1f%%", botManager.currentDrawdown),
                            valueColor: .red)
                        ...
                        MetricRow(
                            ...
                            valueColor: .green)
                        MetricRow(
                            label: "Win Rate", value: String(format: "%.1f%%", botManager.winRate),
                            valueColor: .green)
                        MetricRow(
                            label: "Risk Tier", value: botManager.riskTier, valueColor: .purple)
                    }
                    .padding(20)
                    .background(
                        ZStack {
